...
// Unmarshaller is a XLS to struct unmarshaller
type Unmarshaller struct {
	// should probably put the Excelize handler in here
...
// NewUnmarshaller creates an unmarshaller from a excelize Reader and a struct.
/*func NewUnmarshaller(f *excelize.File, out interface{}) (*Unmarshaller, error) {
...
// XLS headers against the XLS tags in the struct.
func validate(um *Unmarshaller, s interface{}, headers []string) error {
	...
	for i, csvColumnHeader := range headers {
		...
		if fieldInfo := getXLSFieldPosition(csvColumnHeader, structInfo, curHeaderCount); fieldInfo != nil {
...
// unmarshalRow converts a XLS row to a struct, based on XLS struct tags.
