...
// sourced here under MIT License: https://github.com/gocarina/gocsv/blob/master/reflect.go
import (
	"reflect"
	"strings"
	"sync"
...
// fieldInfo is a struct field that should be mapped to a XLS column, or vice-versa
...
func getFieldInfos(rType reflect.Type, parentIndexChain []int) []fieldInfo {
	...
	for i := 0; i < fieldsCount; i++ {
		...
		if field.Type.Kind() == reflect.Ptr && field.Type.Elem().Kind() == reflect.Struct {
			// unless it implements marshalText or marshalXLS. Structs that implement this
